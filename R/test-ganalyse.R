...
#' 
#' @description Runs a set of tests to check if \code{ganalyse} is working 
#' correctly.
#'
...
#' @param verbose If \code{TRUE} sets \code{ganalyse_verbose} to \code{TRUE} for 
...
#' }
test_ganalyse <- function(verbose=FALSE, pkg="pkg", silent=FALSE) {
    ...
    for (fn in file.path(d, 'tests.Rraw')) {    # not testthat
        cat("Running", fn, "\n")
        oldverbose = getOption("ganalyse_verbose")
        if (verbose) options(ganalyse_verbose=TRUE)
        ...
        options(ganalyse_verbose=oldverbose)
    }
    invisible(sum(sapply(envirs, `[[`, "nfail"))==0)
}

# Define test() and its globals here, for use in dev
# But primarily called by test_ganalyse() calling inst/tests/tests.Rraw
...
test <- function(num, x, y, error=NULL, warning=NULL, output=NULL) {
    ...
    #    ganalyse appropriate definition of "equals" different
    ...
    # to cater for both test_ganalyse() and stepping through tests in dev
